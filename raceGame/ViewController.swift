...
class ViewController: UIViewController, FirstViewControllerDelegate {
    func addTestTime(time: String, score: Int) {
        addTime(time, score)
    }
    ...
    func defaultSettings() {
        ...
        addingAccountCountGames()
        addingAccountRecord()
        addTime(AppSettings.shared.timeOfLastRecord, AppSettings.shared.lastScore)
        
    }
    
    func addTime(_ time: String, _ score: Int) {
        guard AppSettings.shared.timeOfLastRecord == "" else {
            return testLabel.text = "\(time) - \(score) points"
        }
    }
